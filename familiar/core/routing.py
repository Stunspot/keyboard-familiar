...
class Router:
    def __init__(
        self, surfaces: dict[str, object], timeout_s: float = 1.0, mirror_debug: bool = True
    ) -> None:
    ...
    async def route_directives(self, directives: list[Directive]) -> list[RenderResult]:
        ...
        for directive in directives:
            ...
            if (
                self.mirror_debug
                and "console_debug" in self.surfaces
                and directive.target_surface != "console_debug"
            ):
            ...
            for target_name in targets:
                ...
                if surface is None:
                    results.append(
                        RenderResult(
                            surface=target_name, directive_id=directive.id, ok=False, detail="surface-missing"
                        )
                    )
                    continue
                ...
                routed = (
                    directive
                    if target_name == directive.target_surface
                    else Directive(
                        kind=directive.kind,
                ...
                try:
                    ...
                    results.append(
                        RenderResult(
                            surface=target_name, directive_id=directive.id, ok=False, detail="render-timeout"
                        )
                    )
