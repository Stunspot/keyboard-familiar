...
class FamiliarApp:
    ...
    def load_runtime(self) -> None:
        ...
        if isinstance(domains, dict):
            self.state._snapshot.domains = domains
    ...
    async def publish_event(self, event: Event) -> list[RenderResult]:
        ...
        self.trace.append(f"event {event.type} from {event.source}")  # compatibility with older tests
        ...
        for brain_name, brain in self.plugins.brains.items():
            ...
            merged.proposals.extend(brain_result.proposals)
            merged.derived_events.extend(brain_result.derived_events)
        ...
        for derived in merged.derived_events:
